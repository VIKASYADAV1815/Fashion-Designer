...
export default function AtelierSpecSheet() {
  ...
  const features = [
    { 
      ...
      svg: (
        <svg viewBox="0 0 100 100" className="w-10 h-10 stroke-amber-200 group-hover:stroke-amber-200 transition-all duration-700 fill-none stroke-[0.5]">
          ...
          <circle cx="50" cy="50" r="1.5" className="fill-amber-200 group-hover:fill-amber-200" stroke="none" />
        </svg>
      )
    },
    { 
      ...
      svg: (
        <svg viewBox="0 0 100 100" className="w-10 h-10 stroke-amber-300 group-hover:stroke-amber-300 transition-all duration-700 fill-none stroke-[0.5]">
          ...
          {/* Shimmer Points */}
          <circle cx="25" cy="30" r="0.8" className="fill-amber-200 opacity-100 group-hover:opacity-100 transition-opacity duration-1000" />
          <circle cx="75" cy="30" r="0.8" className="fill-amber-200 opacity-100 group-hover:opacity-100 transition-opacity duration-700" />
        </svg>
      )
    },
    { 
      ...
      svg: (
        <svg viewBox="0 0 100 100" className="w-10 h-10 stroke-amber-200 group-hover:stroke-amber-200 transition-all duration-700 fill-none stroke-[0.5]">
          ...
          <circle cx="50" cy="50" r="2" className="fill-amber-400 group-hover:fill-amber-400" stroke="none" />
        </svg>
      )
    },
  ];
    ...
    <section ref={sectionRef} className="relative bg-[#0c0c0b] text-stone-200 py-12 md:py-16 overflow-hidden border-y border-stone-900">
      ...
      <div className="container mx-auto px-6 relative z-10">
        <div className="flex flex-col md:flex-row items-center justify-between mb-12 pb-6 border-b border-stone-900 space-y-4 md:space-y-0">
          ...
          <div className="hidden md:block w-px h-12 bg-stone-800 mx-8" />
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
          {features.map((item) => (
            <div key={item.id} className="spec-item group flex flex-col items-center text-center">
              ...
              <div className="relative w-20 h-20 flex items-center justify-center mb-6 border border-amber-200/20 group-hover:border-amber-200/20 transition-all duration-700">
                <div className="absolute inset-1 border border-amber-200/5 group-hover:border-amber-200/5 transition-all duration-700" />
              ...
              <div className="space-y-3 max-w-60">
                <span className="text-[9px] font-sans font-bold uppercase tracking-[0.2em] text-amber-200/70 group-hover:text-amber-200/70 transition-colors">
                  {item.category}
                </span>
                <h3 className="text-lg md:text-xl font-serif text-white group-hover:text-white transition-colors">
                  {item.title}
                </h3>
                <div className="h-px w-10 bg-amber-200/30 mx-auto group-hover:w-10 group-hover:bg-amber-200/30 transition-all duration-500" />
                <ul className="space-y-1.5 pt-1">
                  {item.details.map((detail, idx) => (
                    <li key={idx} className="text-[10px] font-sans uppercase tracking-widest text-stone-300 group-hover:text-stone-300">
                      {detail}
                    </li>
                  ))}
                </ul>
              </div>
              <span className="mt-6 text-[8px] font-mono text-stone-600 tracking-[0.3em] uppercase group-hover:text-stone-600 transition-colors">
                REF_{item.id}
              </span>
            </div>
          ))}
