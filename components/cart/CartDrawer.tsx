    ...
    <AnimatePresence>
      {open && (
        <>
          ...
          />
          <motion.aside
            className="fixed right-0 top-0 h-full w-full sm:w-[420px] bg-black/98 backdrop-blur-3xl text-white z-50 border-l border-white/5 flex flex-col shadow-[0_0_80px_rgba(0,0,0,1)]"
            initial={{ x: "100%" }}
            animate={{ x: 0 }}
            exit={{ x: "100%" }}
          ...
          >
            ...
            <div className="px-6 py-8 flex items-center justify-between border-b border-white/5 bg-black/20">
              <div className="space-y-1">
                <h3 className="text-[10px] uppercase tracking-[0.5em] font-black text-white">Your Selection</h3>
              ...
              <div className="flex items-center gap-5">
                ...
                <button
                  ...
                  onClick={onClose}
                >
                  <X size={18} className="group-hover:rotate-90 transition-transform duration-700 text-gray-400 group-hover:text-white" />
                </button>
              </div>
            </div>
              ...
              {items.length === 0 ? (
                <div className="h-full flex flex-col items-center justify-center space-y-6 opacity-40">
                  ...
                  <div className="text-center space-y-1.5">
                    <p className="text-[9px] uppercase tracking-[0.4em] font-bold">The Bag is Empty</p>
                    <button 
                      onClick={onClose}
                ...
                <div className="divide-y divide-white/5">
                  {items.map((item) => (
                    <div key={item.id} className="flex items-center gap-5 py-6 group first:pt-2">
                      <div className="relative w-16 h-20 overflow-hidden bg-neutral-900 flex-shrink-0 ring-1 ring-white/5">
                        {item.image ? (
                          ...
                          <div className="w-full h-full flex items-center justify-center bg-neutral-900/50">
                             <span className="text-[7px] uppercase tracking-widest text-gray-700">KCB</span>
                          </div>
                        )}
                      </div>
                      <div className="flex-1 min-w-0 space-y-2">
                        ...
                        <div className="flex items-center justify-between text-[9px] font-medium uppercase tracking-[0.1em]">
                          ...
                          <span className="text-white tracking-tighter">₹{(item.price * item.qty).toLocaleString("en-IN")}</span>
                        </div>
                      </div>
                    </div>
                  ))}
              ...
              <div className="px-6 py-8 border-t border-white/5 bg-black/40 backdrop-blur-3xl">
                <div className="space-y-3 mb-6">
                  <div className="flex items-center justify-between">
                    <span className="text-[9px] uppercase tracking-[0.4em] text-gray-600 font-bold">Investment</span>
                ...
                <button className="relative w-full group/checkout overflow-hidden">
                  ...
                  <div className="relative border border-white/10 py-4.5 flex items-center justify-center gap-3 transition-colors duration-500 group-hover/checkout:text-black">
                    <span className="text-[9px] uppercase tracking-[0.6em] font-black">Finalize Order</span>
                  </div>
                </button>
